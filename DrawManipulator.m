...
for i=1:8
%     tmp = uLINK(2).p + uLINK(2).joint_vertex(i,:)';
%     uLINK(2).joint_vertex(i,:) = tmp';
%     tmp = uLINK(2).p + uLINK(2).link_vertex(i,:)';
%     uLINK(2).link_vertex(i,:) = tmp';
    
    tmp = Rodrigues([1 0 0]', 90) * ([0 0 1]' + uLINK(3).joint_vertex(i,:)');
    uLINK(3).joint_vertex(i,:) = tmp';
    tmp = Rodrigues([1 0 0]', 90) * ([0 0 1]' + uLINK(3).link_vertex(i,:)');
    uLINK(3).link_vertex(i,:) = tmp';
    
%     tmp = uLINK(4).p + uLINK(4).joint_vertex(i,:)';
%     uLINK(4).joint_vertex(i,:) = tmp';
%     tmp = uLINK(4).p + uLINK(4).link_vertex(i,:)';
